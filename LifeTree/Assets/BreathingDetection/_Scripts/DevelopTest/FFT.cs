...
public class FFT : MonoBehaviour
{
	...
	void Start()
	{
		...
		for (int i = 0; i < audioObj.Length; i++)
		{
			...
			audioObj[i].SetClip(new AudioClip());
		}
	...
	private void Check()
	{
		...
		audioObj[index].aSource.GetSpectrumData(freqData, 0, FFTWindow.Hamming);
	...
	private IEnumerator StartPlay(AudioClip buffer)
	{
		...
		audioObj[index].aSource.Play();

		audioObj[Mathf.Abs((index % 2) - 1)].aSource.Stop();
